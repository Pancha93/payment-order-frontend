...
import { ActualizarOrdenpagoComponent } from '../../ordenpago/actualizar-ordenpago/actualizar-ordenpago.component';
import { CrearOrdenpagoComponent } from '../../ordenpago/crear-ordenpago/crear-ordenpago.component';
...
@Component({
  selector: 'app-leer-ordenpago',
  templateUrl: './leer-ordenpago.component.html',
  standalone: true,
  imports: [
    FormsModule,
    CommonModule,
  ...
  styleUrls: ['./leer-ordenpago.component.scss']
})
export class LeerOrdenpagoComponent implements OnInit {
  // Columnas que se mostrarán en la tabla
  displayedColumns: string[] = ['id', 'numeroOrden', 'estado', 'facturaUrl', 'facturaImagen', 'fechaCreacion', 'fechaDecision', 'creador', 'acciones'];

  // Array para almacenar los datos de la entidad
  ordenpagos: OrdenpagoComponent[] = [];
   ...
   * @param ordenpagoService Servicio para gestionar la entidad
  ...
  loadData(): void {
    this.ordenpagoService.findAll().subscribe({
      next: data => {
        this.dataSource.data = data.map(item => Object.assign(new OrdenpagoComponent(this.ordenpagoService), item));
      this.dataSource.paginator = this.paginator;
      this.dataSource.sort = this.sort;
    },
       error: error => {
        console.error('Error al obtener ordenpagos:', error);
  ...
  onCreate(): void {
    ...
    const dialogRef = this.dialog.open(CrearOrdenpagoComponent, {
      minWidth: minWidth,
      data: {}
    });
   ...
   * @param {any} ordenpago - El registro a editar
   * @description Abre un diálogo modal para editar el registro seleccionado
   */
  onEdit(ordenpago: any): void {
    const screenWidth = window.innerWidth;
    const minWidth = screenWidth < 600 ? '90vw' : '800px';
    const dialogRef = this.dialog.open(ActualizarOrdenpagoComponent, {
      minWidth: minWidth,
      data: ordenpago,
    });
   ...
   * @param {number} ordenpagoId - ID del registro a eliminar
   * @description Ejecuta el proceso de eliminación con validaciones previas y posteriores
   */
  onDelete(ordenpagoId: number) {
    // Llamada a acciones previas
    const shouldProceed = this.preDelete(ordenpagoId);
    if (!shouldProceed) {
      return;
    }
    try {

      this.ordenpagoService.deleteById(ordenpagoId).subscribe(
        () => {
          // Llamada a acciones posteriores
          this.postDelete(ordenpagoId);
        },
        error => {
  ...
  exportToExcel(): void {
    try {
      ...
      XLSX.writeFile(wb, 'ordenpago_datos.xlsx');
  ...
  onDownload(element: any) {
    this.ordenpagoService.getFilesByOrdenpagoId(element.id)
      .subscribe({
        next: (files) => {
          ...
          dialogRef.afterClosed().subscribe((selectedFiles: string[]) => {
            if (selectedFiles && selectedFiles.length > 0) {
              selectedFiles.forEach(selectedFile => {
                this.ordenpagoService.downloadFile(selectedFile)
                  .subscribe(blob => {
  ...
  onShowFacturaUrl(element: any) {
    const rawPaths = element.facturaUrl;
    if (!rawPaths) {
      this.showMessage('No hay archivos de FacturaUrl.', 'error');
      return;
    }
  ...
  onShowFacturaImagen(element: any) {
    const rawPaths = element.facturaImagen;
    if (!rawPaths) {
      this.showMessage('No hay archivos de FacturaImagen.', 'error');
      return;
    }
    let files = rawPaths.split(',').map((path: string) => path.trim());
    files = files.map((path: string) => this.extractFileName(path));
